...
def send_ubuntu_notification(title, message):
    result = subprocess.run(['notify-send', title, message], capture_output=True, text=True)
    print(f"Notification result: {result.stdout}, {result.stderr}")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if current_state != previous_state:
            if current_state:
                send_ubuntu_notification("Website Active", f"The web app at {url} is running.")
            else:
                send_ubuntu_notification("Website Inactive", f"The web app at {url} is not running.")
